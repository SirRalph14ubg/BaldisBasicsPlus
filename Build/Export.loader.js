...
function createUnityInstance(r, n, l) {
    ...
    return c.SystemInfo = function () {
        ...
        if (r && r.onProgress) onProgress = r.onProgress;
        let url = e;
        if (e.includes("StreamingAssets")) {
            url = "StreamingAssets/" + e.split("/").pop();
        }
        return fetch(url, r).then(function(resp) {
            return c.readBodyWithProgress(resp, onProgress, r && r.enableStreamingDownload);
        });
